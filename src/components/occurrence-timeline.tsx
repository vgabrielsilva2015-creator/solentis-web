  ...
  return (
    <div className="space-y-6">
      <h3 className="text-sm font-bold text-slate-350 uppercase tracking-wider flex items-center gap-2">
        <Clock className="w-4 h-4 text-slate-400" /> Timeline e Comentários
      </h3>
      ...
      <div className="relative border-l border-slate-800 ml-4 pl-6 space-y-6">
        {timelineEvents.map((evt) => {
          ...
          let iconBg = 'bg-slate-900 border-slate-800'
          let contentBg = 'bg-slate-950/20'
          ...
          return (
            <div key={evt.id} className="relative group">
              ...
              {/* Event card */}
              <div className={`rounded-xl border border-slate-850 p-4 space-y-2 ${contentBg}`}>
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1">
                  <div className="flex items-center gap-1.5">
                    <span className="text-xs font-bold text-slate-200">{evt.title}</span>
                    {evt.actorRole && (
                      <Badge className="bg-slate-800 text-slate-400 text-[9px] hover:bg-slate-800">
                        {evt.actorRole}
                      </Badge>
                    )}
                  </div>
                ...
                <div className="space-y-1">
                  {evt.content && (
                    <p className="text-xs text-slate-300 whitespace-pre-wrap leading-relaxed bg-slate-900/40 rounded p-2.5 border border-slate-850/50">
                      {evt.content}
                    </p>
                  )}
                  <p className="text-[10px] text-slate-500 flex items-center gap-1.5 pt-0.5">
                    <User className="w-3 h-3" /> Por: <span className="font-semibold text-slate-400">{evt.actor}</span>
                  </p>
                </div>
              </div>
            </div>
      ...
      {/* Add comment form */}
      <form onSubmit={handleAddComment} className="border-t border-slate-850 pt-4 mt-6">
        {error && (
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 bg-slate-900 border border-slate-800 rounded-lg px-3 py-2 text-xs text-slate-200 placeholder:text-slate-600 focus:outline-none focus:border-slate-700"
          />
          <Button
            type="submit"
            disabled={isPending || !commentText.trim() || commentText.trim().length < 2}
            className="bg-slate-100 hover:bg-white text-slate-900 text-xs px-3 h-9 flex items-center gap-1"
          >
            {isPending ? (
