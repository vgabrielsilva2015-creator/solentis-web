...
export default async function HistoricoLeituraPage({
  searchParams,
}: {
  ...
  return (
    <main className="mx-auto max-w-lg px-4 py-6 space-y-5">
        ...
        <div className="space-y-1">
          <Link href="/operador/leituras" className="inline-flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors">
            <ArrowLeft className="h-3.5 w-3.5" />
            Voltar ao checklist
          </Link>
          <h1 className="text-xl font-semibold">Histórico de Leituras</h1>
          <p className="text-xs text-muted-foreground">{total} registro(s) no total</p>
        </div>

        {/* Filtros rápidos */}
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
          <Link href="/operador/leituras/historico" className={`shrink-0 rounded-full px-3 py-1 text-xs font-medium border ${!filter ? 'bg-brand/20 text-brand border-brand/30' : 'bg-card border-border text-muted-foreground hover:bg-muted'}`}>
            Todas
          </Link>
          <Link href="/operador/leituras/historico?filter=conformant" className={`shrink-0 rounded-full px-3 py-1 text-xs font-medium border ${filter === 'conformant' ? 'bg-brand/20 text-brand border-brand/30' : 'bg-card border-border text-muted-foreground hover:bg-muted'}`}>
            Conforme
          </Link>
          <Link href="/operador/leituras/historico?filter=non-conformant" className={`shrink-0 rounded-full px-3 py-1 text-xs font-medium border ${filter === 'non-conformant' ? 'bg-red-900/30 text-red-400 border-red-900/50' : 'bg-card border-border text-muted-foreground hover:bg-muted'}`}>
            Não Conforme
          </Link>
        </div>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between pt-1 text-sm">
            {page > 1 ? (
              <Link
                href={`/operador/leituras/historico?page=${page - 1}${filter ? `&filter=${filter}` : ''}`}
