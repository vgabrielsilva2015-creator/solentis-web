      ...
      <div className="p-4 space-y-3">
        <div className="flex items-start gap-3">
          ...
          <div className={[
            ...
            isSkipped ? 'border-border bg-secondary'   :
            ...
            {isSkipped && (
              <svg className="h-2.5 w-2.5 text-muted-foreground" viewBox="0 0 10 10" fill="none">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-1.5">
              <p className={['text-sm font-medium leading-snug', isDone || isSkipped ? 'text-muted-foreground' : 'text-foreground'].join(' ')}>
                {task.title}
              </p>
            ...
            {isPendingStatus && !task.requires_photo && (
              <p className="mt-1 text-xs text-muted-foreground">📷 Aceita foto de comprovação</p>
            )}
          </div>
        </div>
            ...
            {showOriginal && (
              <div className="mt-1.5 rounded-lg border border-sky-900/30 bg-sky-950/20 px-3 py-2.5 space-y-1.5">
                ...
                {task.repeatedFrom.completion_notes && (
                  <p className="text-xs text-muted-foreground leading-relaxed">{task.repeatedFrom.completion_notes}</p>
                )}
        ...
        {isDone && (
          <div className="ml-8 rounded-lg border border-green-900/30 bg-green-950/20 px-3 py-2.5 space-y-1.5">
            ...
            {task.completion_notes && (
              <p className="text-xs text-muted-foreground leading-relaxed">{task.completion_notes}</p>
            )}
        ...
        {canAct && !expanded && (
          <div className="ml-8 flex gap-2">
            ...
            <form action={pularTarefa.bind(null, task.id)}>
              <Button type="submit"
                className="h-12 border border-border bg-muted text-muted-foreground hover:bg-secondary text-sm px-5"
               disabled={isPending}>
                Pular
              </Button>
            </form>
        ...
        {canRepeat && repeating && (
          <form action={repeatAction} className="ml-8 space-y-2 rounded-lg border border-sky-900/40 bg-sky-950/10 p-3">
            ...
            <textarea
              ...
              className="w-full resize-none rounded-lg border border-border bg-muted px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-sky-600 focus:outline-none"
            />
            {repeatState.error && <p className="text-xs text-red-400">{repeatState.error}</p>}
            <div className="flex gap-2">
              <Button type="button" onClick={() => setRepeating(false)}
                className="h-9 border border-border bg-muted text-muted-foreground hover:bg-secondary text-sm px-4">
                Cancelar
              </Button>
      ...
      {expanded && canAct && (
        ...
        >
          <textarea
            ...
            className="w-full resize-none rounded-lg border border-border bg-muted px-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:border-emerald-600 focus:outline-none"
          />

          <div className="space-y-1.5">
            <label className={['text-xs font-medium', task.requires_photo ? 'text-amber-400' : 'text-muted-foreground'].join(' ')}>
              Fotos comprovação{' '}
              {task.requires_photo ? (
                <span className="font-normal">— obrigatória para concluir</span>
              ) : (
                <span className="font-normal text-muted-foreground">até 3 · opcional</span>
              )}
            </label>
            <input
              name="photos"
