    ...
    <main className="mx-auto max-w-2xl px-4 py-6 space-y-6">
        ...
        <form method="GET" className="flex flex-wrap gap-3 items-end">
          ...
          <div className="space-y-1.5">
            <label htmlFor="days" className="text-sm font-medium text-foreground">Período</label>
            <select
              id="days" name="days"
              defaultValue={String(days)}
              className="rounded-md border border-border bg-muted text-foreground px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-ring"
            >
          ...
          <div className="rounded-xl border border-border bg-card p-4 space-y-3">
            <div className="flex items-baseline justify-between">
              <h2 className="text-base font-medium">
                {selectedParam.name}
                <span className="ml-2 text-sm font-normal text-muted-foreground">{selectedParam.unit}</span>
              </h2>
              <span className="text-xs text-muted-foreground">{dataPoints.length} medição(ões)</span>
            </div>
            <AnalysisChart
