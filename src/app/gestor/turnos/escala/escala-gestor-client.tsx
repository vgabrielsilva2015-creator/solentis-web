  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-800 pb-4">
        <div className="flex items-center gap-2">
          <Link href="/gestor/turnos">
            <Button variant="outline" className="border-slate-850 hover:bg-slate-900 text-xs gap-1.5 h-9">
        ...
        <div className="flex items-center gap-2 bg-slate-900 border border-slate-800 rounded-lg p-1 w-full sm:w-auto justify-between sm:justify-start">
      ...
      <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-4 space-y-3">
        <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Legendas & Turnos</h3>
      ...
      <div className="bg-slate-900/40 border border-slate-800 rounded-xl overflow-hidden shadow-2xl relative">
        {/* Loading overlay */}
        {isPending && (
          <div className="absolute inset-0 bg-slate-955/60 backdrop-blur-[1px] z-20 flex items-center justify-center">
            <div className="flex items-center gap-2 bg-slate-900 border border-slate-850 p-4 rounded-xl shadow-xl">
              <Loader2 className="w-5 h-5 text-emerald-400 animate-spin" />
              <span className="text-sm font-semibold text-slate-200">Atualizando escala...</span>
            </div>
          </div>
        )}

        {/* Days of week */}
        <div className="grid grid-cols-7 border-b border-slate-850 bg-slate-900/80 text-center py-2.5 text-xs font-bold text-slate-400">
                ...
                <div className="mt-2 w-full space-y-1">
                  {!isMaintenanceDay ? (
                    dayScales.map((s: any) => {
                      ...
                      return (
                        <div key={s.id} className="text-[10px] flex items-center gap-1 font-medium bg-slate-900/80 px-1 py-0.5 rounded border border-slate-800 text-slate-300">
                          <span className={`h-1.5 w-1.5 rounded-full ${style.dot}`} />
                          <span className="font-bold text-slate-400">{style.abbreviation}:</span>
                  ...
                  {totalTasks > 0 && (
                    <div className="text-[9px] font-bold text-slate-500 flex items-center gap-0.5 mt-1 pt-1 border-t border-slate-800/40">
      ...
      {selectedDateStr && selectedCellData && (
        <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/80 backdrop-blur-sm animate-fade-in">
          <div className="w-full max-w-lg h-full bg-slate-900 border-l border-slate-800 shadow-2xl flex flex-col overflow-hidden animate-slide-in">
            {/* Header */}
            <div className="p-5 border-b border-slate-800 flex items-center justify-between bg-slate-900/90 sticky top-0 z-10">
              <div className="flex items-center gap-2">
                <Calendar className="w-5 h-5 text-emerald-500" />
                <div>
                  <h2 className="text-base font-bold text-slate-200">Editar Escala & Tarefas</h2>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-6">
              ...
              <div className="bg-slate-950/40 border border-slate-850 rounded-xl p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-slate-300">Status de Funcionamento</span>
                  </div>
                  <Button
                ...
                {selectedCellData.isMaintenanceDay ? (
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Descrição do bloqueio (opcional)</label>
                    <input
                      type="text"
                      placeholder="Ex: Limpeza do reator biológico..."
                      value={maintenanceDesc}
                      onChange={(e) => setMaintenanceDesc(e.target.value)}
                      className="w-full bg-slate-900 border border-slate-800 rounded-lg px-3 py-2 text-xs text-slate-200 placeholder:text-slate-600 focus:outline-none focus:border-slate-700"
                    />
                  </div>
                )}
              </div>
              ...
              <div className="space-y-3">
                ...
                {selectedCellData.isMaintenanceDay ? (
                  <p className="text-xs text-slate-500 italic">Turnos de operação suspensos para este dia de manutenção.</p>
                ) : (
                  <div className="space-y-3">
                    {shifts.map((s) => {
                      ...
                      return (
                        <div key={s.id} className="p-4 rounded-xl border border-slate-850 bg-slate-950/30 space-y-3">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                          ...
                          <div className="space-y-1.5">
                            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Atribuir Operador</label>
                            <select
              ...
              <div className="space-y-3">
                ...
                {selectedCellData.isMaintenanceDay ? (
                  <p className="text-xs text-slate-500 italic">Não é possível gerenciar tarefas em dias de manutenção programada.</p>
                ) : (
                  <div className="space-y-4">
                    {shifts.map((s) => {
                      ...
                      return (
                        <div key={s.id} className="p-4 rounded-xl border border-slate-850 bg-slate-950/20 space-y-3">
                          <h4 className="text-xs font-bold text-slate-400">Turno: {s.name}</h4>

                          {/* Existing Tasks */}
                          {tasksList.length > 0 ? (
                            <div className="space-y-2">
                              {tasksList.map((task: any) => (
                                <div key={task.id} className="flex items-center justify-between p-2.5 rounded-lg border border-slate-800/80 bg-slate-900/50">
                            ...
                            <p className="text-[11px] text-slate-500 italic pl-1">Nenhuma tarefa cadastrada para este turno.</p>
                          )}

                          {/* Inline Form to Add Task */}
                          <div className="pt-2 border-t border-slate-800/40 space-y-2">
                            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Nova Tarefa</p>
                            <div className="space-y-2">
                              <input
                                type="text"
                                ...
                                className="w-full bg-slate-900 border border-slate-800 rounded-lg px-2.5 py-1.5 text-xs text-slate-200 placeholder:text-slate-650 focus:outline-none focus:border-slate-700"
                              />
                              <input
                                type="text"
                              ...
                              <div className="flex gap-2">
                                <select
                                  ...
                                  className="flex-1 bg-slate-900 border border-slate-800 rounded-lg px-2 py-1.5 text-xs text-slate-300 focus:outline-none focus:border-slate-700"
                                >
                                ...
                                <Button
                                  ...
                                  className="bg-slate-100 hover:bg-white text-slate-900 text-xs px-3 h-8 flex items-center gap-1"
                                >
              ...
              <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid gap-2">
                      {selectedCellData.daySchedules.map((s) => (
                        <div key={s.id} className="p-3 rounded-lg border border-slate-800 bg-slate-950/40 text-xs">
                          <div className="flex justify-between font-bold">
                            <span className="text-slate-200">{s.collection_point.name}</span>
                            <span className="text-slate-400">{s.parameter.name}</span>
                          </div>
                          <p className="text-[10px] text-slate-500 mt-0.5">Executor: {s.executor_role === 'OPERATOR' ? 'Operador' : 'Técnico'}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                    ...
                    <div className="grid gap-2">
                      {selectedCellData.dayOccurrences.map((o) => (
                        <div key={o.id} className="p-3 rounded-lg border border-red-950/30 bg-red-950/5 text-xs space-y-0.5">
                          <div className="flex justify-between font-bold">
                            <span className="text-slate-200">{o.collection_point?.name || 'Geral'}</span>
                            <span className="text-red-400">{o.severity}</span>
                          </div>
                          <p className="text-slate-300">{o.description}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {selectedCellData.daySchedules.length === 0 && selectedCellData.dayOccurrences.length === 0 && (
                  <p className="text-xs text-slate-500 italic">Nenhum evento ou análise agendada para esta data.</p>
                )}
              </div>
