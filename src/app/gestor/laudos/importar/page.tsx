          ...
          {filesQueue.length > 0 && (
            <div className="bg-card border border-border rounded-xl overflow-hidden">
              <div className="px-4 py-3 border-b border-border bg-background/50">
                <h4 className="text-sm font-medium text-foreground">Fila de Processamento</h4>
              </div>
              ...
              {!isProcessing && (successfulFiles.length > 0 || filesQueue.some(f => f.status === 'error')) && (
                <div className="p-4 bg-background/30 flex justify-between items-center">
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  {item.data?.resultados?.map((p: any, pIdx: number) => {
                    ...
                    return (
                      <div key={pIdx} className={`flex flex-col md:flex-row items-center gap-4 p-3 rounded-lg border ${isUnmapped ? 'bg-amber-950/20 border-amber-800/40' : 'bg-background border-border'}`}>
                        <div className="flex-1 w-full">
                          <div className="text-sm font-medium text-foreground">
                            {p.parametro} <span className="text-muted-foreground font-normal">({p.unidade})</span>
                          </div>
                          <div className="text-xl font-bold text-white mt-1">{p.bruto || p.valor}</div>
                        </div>
                        <div className="hidden md:flex items-center justify-center px-2">
                          <svg className="w-5 h-5 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
                        </div>
