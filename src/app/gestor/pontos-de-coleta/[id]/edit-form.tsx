...
import { editarPontoColeta, toggleAtivoPontoColeta, type PontoColetaFormState } from '../actions'
...
export function EditPontoColetaForm({ ponto }: { ponto: PontoColeta }) {
  ...
  function handleToggle() {
    ...
    startToggle(async () => {
      await toggleAtivoPontoColeta(ponto.id)
      router.refresh()
    })
  }
    ...
    <main className="px-6 py-8 space-y-6 max-w-2xl">
      ...
      <div className="rounded-xl border border-border bg-card p-6">
        ...
        <Button type="button" variant="outline" disabled={isPendingToggle} onClick={handleToggle}
          ...
          {isPendingToggle ? 'Aguarde…' : ponto.is_active ? 'Desativar ponto de coleta' : 'Reativar ponto de coleta'}
        </Button>
      </div>
    </main>
  )
